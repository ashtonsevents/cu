    ...
    try {
        ...
        for (let i = 0; i < slidesContent.length; i++) {
            ...
            if (i === 0) {
                ...
                ctx.font = "bold 28px Arial"; // Font for slide 1
                ctx.textAlign = "left";
                ...
                // Handle slides 2-6 with text color #07485B and center the text
                ctx.fillStyle = "#07485B";  // Text color for slides 2–6
                ctx.font = "bold 18px Arial"; // Font for slides 2-6
                ctx.textAlign = "center";
  
                // Split the content by new lines for multiple lines of text
                const lines = content.split('\n');
                let lineHeight = 25; // Space between lines
                let yPosition = 250; // Starting position for text
  
                // Render each line of text for slides 2-6
                lines.forEach((line, index) => {
                    ctx.fillText(line, canvas.width / 2, yPosition + (lineHeight * index)); // Center-align with line spacing
                });
            }
